...
(define-public (tip-driver (job-id uint) (tip-amount uint))
  (let (
    (job (unwrap! (map-get? jobs job-id) ERR_JOB_NOT_FOUND))
    (driver (unwrap! (get driver job) ERR_NOT_DRIVER))
  )
    (asserts! (is-eq tx-sender (get shipper job)) ERR_NOT_SHIPPER)
    (asserts! (is-eq (get status job) "completed") ERR_INVALID_STATUS)
    (asserts! (> tip-amount u0) ERR_INSUFFICIENT_PAYMENT)
    (asserts! (>= (stx-get-balance tx-sender) tip-amount) ERR_INSUFFICIENT_PAYMENT)
    (try! (stx-transfer? tip-amount tx-sender driver))
    (ok true)
  )
)
